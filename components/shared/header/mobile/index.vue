<template>
  <div class="hidden md:block  ">
    ...
    <USlideover v-model="showNavbar"  dir="ltr" :ui="{
      ...
      <div class="flex flex-col p-4" dir="rtl">
        ...
        <!-- Categories -->
        <div>
          <details v-for="item in utilStore.categories" :key="item.id"
            ...
            <summary class="flex cursor-pointer items-center justify-between rounded-lg  py-2">
              <span class="flex items-center gap-x-4">
                <span v-if="item.children" class="w-0.5 h-5 bg-sky-600" ></span>
                <span v-else class="w-0.5 h-5"></span>
                <span class="text-slate-500 dark:text-slate-400 text-sm">
                {{ item.display_title }}
                </span>
              </span>
            ...
            <ul v-if="item.children" class="mt-2  px-4 divide-y-[0.5px] divide-gray-200 dark:divide-gray-800">
              ...
              <li>
                <details v-for="child in item.children" :key="child.id" class="group [&_summary::-webkit-details-marker]:hidden py-2">
                  <summary class="flex cursor-pointer items-center justify-between rounded-lg  py-2">
                       <span class="flex items-center gap-x-4">
                              <span v-if="child.products" class="w-0.5 h-5 bg-sky-600" ></span>
                              <span v-else class="w-0.5 h-5"></span>
                        <span class="text-slate-500 dark:text-slate-400  text-sm">
                        {{ child.display_title }}
                        </span>
