  ...
  <div class="flex items-center flex-col gap-y-4">
    ...
    <div class="max-w-[400px]  ">
      ...
      <LegoPreview class="transition-transform ease-in-out duration-500 outline-none ">
        ...
        />
        <nuxt-img :src="GetImageUrl(image)" alt="product" class="w-full h-auto rounded-lg  "
                  placeholder/>
      </LegoPreview>
    </div>
    <ClientOnly>
      <div v-if="!timerExpired && !fetchPending" class="flex flex-col justify-center items-center gap-y-2 ">
        ...
        <div class="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700 flex items-center max-w-[280px]"
             style="direction: ltr">
          <div class="bg-red-600 dark:bg-red-500 h-1.5 rounded-full transition-all  duration-1000 "
...
const props = defineProps<{
  ...
  longest_special_price_end_date?: string | any,
  longest_special_price_start_date?: string | any,
}>()

const startDate = new Date(props.longest_special_price_start_date)
const endDate = new Date(props.longest_special_price_end_date)
const remainingTimeAsDate = new Date(props.longest_special_price_end_date)
